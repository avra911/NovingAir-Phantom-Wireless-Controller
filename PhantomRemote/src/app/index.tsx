...
export default function Index() {
  ...
  const getFluxIcon = (flux: PhantomState['flux']) => {
    switch (flux) {
      case 'RECOVERY': return "sync";
      case 'SLAVE_MASTER': return "sync-off";
      case 'INTAKE': return "arrow-down-bold";
      case 'EXTRACT': return "arrow-up-bold";
      default: return "sync";
    }
  };
  ...
  */
  ...
  const getCO2Color = (ppm: number) => {
    if (ppm < 800) return "#00ffcc";      // Good
    if (ppm < 1200) return "#f39c12";     // Moderate
    return "#e74c3c";                     // Unhealthy
  };

  const getTempColor = (temp: number) => {
    if (temp >= 18 && temp <= 24) return "#00ffcc";  // Ideal
    if (temp >= 15 && temp <= 28) return "#f39c12";  // Acceptable
    return "#e74c3c";                                 // Uncomfortable
  };

  const getHumidityColor = (hum: number) => {
    if (hum >= 30 && hum <= 50) return "#00ffcc";    // Ideal
    if (hum >= 25 && hum <= 60) return "#f39c12";    // Acceptable
    return "#e74c3c";                                 // Poor (too dry or too humid)
  };

  const getPMColor = (pm: number) => {
    if (pm < 12) return "#00ffcc";        // Good
    if (pm < 35.4) return "#a8e6cf";      // Moderate
    if (pm < 55.4) return "#f39c12";      // Unhealthy for sensitive
    return "#e74c3c";                     // Unhealthy
  };

  const getTVOCColor = (tvoc: number) => {
    if (tvoc < 0.3) return "#00ffcc";     // Good
    if (tvoc < 1.0) return "#f39c12";     // Acceptable
    return "#e74c3c";                     // Poor
  };

  const getFormaldehyteColor = (hcho: number) => {
    if (hcho < 0.05) return "#00ffcc";    // Safe
    if (hcho < 0.1) return "#f39c12";     // Caution
    return "#e74c3c";                     // Unhealthy
  };
  ...
  if (initialFetching) {
    return (
      <SafeAreaView style={styles.container}>
        ...
        <Text style={[styles.lcdText, { marginTop: 16 }]}>Connecting to backend...</Text>
      </SafeAreaView>
    );
  }
      ...
      <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.lcdScreen}>
          ...
          <View style={styles.lcdRow}>
            ...
            <View style={styles.indicatorBlock}>
              <MaterialCommunityIcons 
                name={phantomState.mode === 'AUTO' ? "brightness-auto" : phantomState.mode === 'SLEEP' ? "sleep" : "gesture-tap"}
                size={22} 
                color="#00ffcc" 
              />
              <Text style={styles.lcdText}>{phantomState.mode}</Text>
            </View>
            ...
            <View style={styles.indicatorBlock}>
              <MaterialCommunityIcons 
                name="fan" 
                size={22} 
                color={phantomState.mode === 'MANUAL' ? "#00ffcc" : "#112a2a"} 
              />
              <Text style={[styles.lcdText, phantomState.mode !== 'MANUAL' && styles.disabledText]}>
          ...
          <View style={styles.lcdRow}>
            ...
            <View style={styles.indicatorBlock}>
              ...
              <Text style={styles.lcdText}>
                {phantomState.flux === 'SLAVE_MASTER' ? 'S-MASTER' : phantomState.flux}
              </Text>
            </View>
            ...
            <MaterialCommunityIcons 
              name="weather-night" 
              size={22} 
              color={phantomState.night ? "#00ffcc" : "#112a2a"} 
            />
            ...
            <MaterialCommunityIcons 
              name="lightning-bolt" 
              size={22} 
              color={phantomState.boost ? "#00ffcc" : "#112a2a"} 
            />
          </View>
        </View>
